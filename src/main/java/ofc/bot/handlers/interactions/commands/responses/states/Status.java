...
public enum Status implements InteractionResult {
    ...
    AUTOPAY_SUBSCRIPTION_CREATED_SUCCESSFULLY(  State.OK, true, "Inscrição para débito automático em `%s` criado com sucesso!"),
    ...
    TWITCH_NO_USERS_FOUND(                      State.FAILURE, true,  "Nenhum usuário encontrado!"),
    TWITCH_USER_LIKELY_ALREADY_SUBSCRIBED(      State.FAILURE, false, "A inscrição deste canal já existe em nossos registros! Se você persiste tendo problemas em não ser notificado, remova a inscrição com `/twitch unsubscribe` e se inscreva novamente."),
    TWITCH_USER_NOT_SUBSCRIBED(                 State.FAILURE, false, "Esta inscrição não existe."),
    ...
    AUTOPAY_SUBSCRIPTION_DOES_NOT_EXIST(        State.FAILURE, false, "> ❌ Você não tem nenhuma inscrição para débito automático com o tipo fornecido."),
    AUTOPAY_SUBSCRIPTION_ALREADY_EXISTS(        State.FAILURE, false, "> ⚠️ Você já tem uma inscrição para débito automático com o tipo informado."),
