...
- Dashboard modules include Birthdays, backed by the existing `birthdays` table; hovering a birthday date shows the person's current age through the shared tooltip; Tickets, backed by existing `support_tickets`, `messages_versions`, and `users` rows; and Economy action-cost configuration, backed by `store_item_settings`. Add schema changes through `database/migrations/`; the dashboard must not create tables at startup.
