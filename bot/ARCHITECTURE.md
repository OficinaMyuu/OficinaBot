...
## Overview
Oficina is a Java 21 Discord bot built on JDA. In the OficinaServices mono-repo it lives under `bot/`. The application boots from `src/main/java/ofc/bot/Main.java`, initializes SQLite-backed persistence, registers Quartz jobs, then wires Discord listeners and slash commands through centralized registries.
...
- CI deploy workflow is defined in `../.github/workflows/deploy.yml` and runs from the `bot/` working directory
