...
public class GiveawayEntryRepository extends Repository<GiveawayEntry> {
    ...
    public List<String> removeUserFromVoiceLockedGiveawaysReturningIds(long userId, long voiceChannelId) {
        var giveaways = ofc.bot.domain.tables.GiveawaysTable.GIVEAWAYS;
        List<String> giveawayIds = ctx.select(GIVEAWAY_ENTRIES.GIVEAWAY_ID)
                .from(GIVEAWAY_ENTRIES)
                .join(giveaways)
                .on(GIVEAWAY_ENTRIES.GIVEAWAY_ID.eq(giveaways.GIVEAWAY_ID))
                .where(GIVEAWAY_ENTRIES.USER_ID.eq(userId))
                .and(giveaways.REQUIRED_VOICE_CHANNEL_ID.eq(voiceChannelId))
                .and(giveaways.STATUS.eq(ofc.bot.domain.entity.enums.GiveawayStatus.ACTIVE.name()))
                .fetch(GIVEAWAY_ENTRIES.GIVEAWAY_ID);

        if (giveawayIds.isEmpty()) return List.of();

        ctx.deleteFrom(GIVEAWAY_ENTRIES)
                .where(GIVEAWAY_ENTRIES.USER_ID.eq(userId))
                .and(GIVEAWAY_ENTRIES.GIVEAWAY_ID.in(giveawayIds))
                .execute();
