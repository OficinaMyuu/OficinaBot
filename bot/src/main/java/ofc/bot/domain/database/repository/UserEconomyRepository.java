...
import ofc.bot.domain.database.MapperFactory;
...
public class UserEconomyRepository extends Repository<UserEconomy> {
    ...
    public int addBankBalances(Map<Long, Integer> values, long timestamp) {
        if (values.isEmpty()) return 0;

        List<Query> queries = values.entrySet()
                .stream()
                .filter(entry -> entry.getValue() != 0)
                .<Query>map(entry -> {
                    long userId = entry.getKey();
                    int amount = entry.getValue();
                    int insertedBank = Math.max(0, amount);

                    return ctx.insertInto(USERS_ECONOMY)
                            .set(USERS_ECONOMY.USER_ID, userId)
                            .set(USERS_ECONOMY.BANK, insertedBank)
                            .set(USERS_ECONOMY.WALLET, 0)
                            .set(USERS_ECONOMY.CREATED_AT, timestamp)
                            .set(USERS_ECONOMY.UPDATED_AT, timestamp)
                            .onDuplicateKeyUpdate()
                            .set(
                                    USERS_ECONOMY.BANK,
                                    DSL.least(
                                            USERS_ECONOMY.BANK.plus(amount),
        ...
        return java.util.Arrays.stream(ctx.batch(queries).execute()).sum();
    }
