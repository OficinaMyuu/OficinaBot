...
public enum Status implements InteractionResult {
    ...
    WORK_SUCCESSFUL_BOOSTING(                   State.OK, true, "> 👑 Eita 🥵 parabéns pelo seu trabalho, você ganhou `$%s` no total hoje, e houve um acréscimo de `$%s` por ser booster."),
    ...
    WAKING_UP_USER_SUCCESSFULLY(                State.OK, true, "> Iniciando processo de mover usuário."),
    ...
    EMOJI_PERMISSION_SUCCESSFULLY_GRANTED(      State.OK, true, "✅ Permissão ao `%s` concedida com sucesso à %s."),
    EMOJI_PERMISSION_SUCCESSFULLY_REVOKED(      State.OK, true, "⚠️ Permissão ao `%s` revogada de %s!"),
    ...
    GROUP_EMOJI_ALREADY_IN_USE(                 State.FAILURE, true,  "Este emoji já está sendo usado por outro grupo."),
    INVALID_DURATION_PROVIDED(                  State.FAILURE, true,  "Duração inválida."),
    PERIOD_TOO_SHORT(                           State.FAILURE, false, "Período curto demais!"),
    GAME_ALREADY_RUNNING_IN_CHANNEL(            State.FAILURE, false, "> ❌ Já existe um mini-game neste canal."),
    ...
    USER_NOT_CONNECTED_TO_VOICE_CHANNELS(       State.FAILURE, false, "> ❌ O usuário informado não está em nenhum canal de voz."),
    ALREADY_WAKINGUP_OTHER_USERS(               State.FAILURE, false, "> ❌ Já tem outro usuário sendo movido."),
