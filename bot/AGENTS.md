...
## Purpose
This file is a low-token index for future agents working in the `bot/` service.
...
- Mono-repo location: this service lives under `bot/` in the OficinaServices repository.
...
- CI currently runs from `bot/`, builds with `-DskipTests`, then uploads `target/bot.jar` through SFTP.
