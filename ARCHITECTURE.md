...
`/blackjack` is a temporary Administrator-only standalone Blackjack command against the bot/dealer, modeled after UnbelievaBoat's public rules. It uses the same bank-only stake range as roulette, 100 through 1,000,000, and the same amount parser for values such as `all`, `2k`, and `750k`. The game uses a shuffled three-deck shoe, deals two player cards and two dealer cards, hides the dealer hole card until settlement, and renders all player-facing embed text and buttons in pt-BR. The active controls are `Pedir Carta`, `Parar`, `Dobrar`, and `Dividir`; there is intentionally no Help button and no player-vs-player invite flow.
