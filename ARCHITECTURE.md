...
- `store_item_settings` stores one mutable, non-negative price per `StoreItemType` key. The migration seeds every supported key; the bot never falls back to a hard-coded price when a row is missing.
...
Group command costs are internal action pricing, not a user-facing shop. `StoreItemType` remains the fixed Java identity and behavior list, while `store_item_settings` owns its mutable price. `/group create`, channel creation, group modifications, bot additions, paid member slots, permissions, and pins resolve the database price when they create the 30-second confirmation; the quoted amount is retained through confirmation and persisted in `groups_perks.value_paid` as historical data. A missing settings row fails the action closed instead of using a code default.
