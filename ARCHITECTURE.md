...
`MessageRenderer` is a generic chronological message-list renderer. Each message-related TSX file contains one component and owns its CSS module. Reply messages always start a full author group, expose stable `message-{snowflake}` DOM targets and an optional reference-selection callback, and render their connector from the top of the current author's avatar. Transcript containers, rather than the renderer, own newest-at-bottom initialization and backward pagination. Discord PNG/APNG stickers load from the CDN; standard Lottie stickers fall back through the authenticated backend asset route and lazy-load the light Lottie player with reduced-motion behavior.
