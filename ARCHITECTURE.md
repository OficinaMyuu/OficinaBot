...
The Tickets dashboard page keeps ticket-list query orchestration in `frontend/src/pages/tickets/TicketsPage.tsx`. Its toolbar, list, ticket row, and expanded details are focused colocated components, each with an isolated CSS module. A ticket transcript passes the ticket payload's `channel_id` to the generic `useChannelMessages` hook; tickets do not own a separate message API or message model. Generic message types, API calls, user mapping, and scroll anchoring live under `frontend/src/types/message.ts`, `frontend/src/services/messageService.ts`, `frontend/src/utils/messageUtils.ts`, and `frontend/src/hooks/` so a future channel-log surface can reuse the same stack.

`MessageRenderer` is a generic chronological message-list renderer. Each message-related TSX file contains one component and owns its CSS module. `MessageGroup` wraps consecutive messages from the same author so condensed rows remain contiguous while distinct groups retain larger spacing; replies always start a full author group, expose stable `message-{snowflake}` DOM targets, and render their connector with a small clearance above the current author's avatar. Group-leading bot messages use the `is_bot` user field to place a Discord-style `BOT` badge between the author name and timestamp. Reply previews identify the referenced author with a compact avatar and `@display name`, then navigate to loaded targets or reload the transcript around an unloaded target, after which the transcript can paginate in both directions with `before` and `after`. Sticker-only references use a compact attachment affordance instead of empty-text copy. Transcript containers, rather than the renderer, own newest-at-bottom initialization and pagination. Unicode and custom Discord emojis render at message scale when surrounded by text and at a larger scale when they are the only content. Discord asset URL construction is centralized in `frontend/src/config/discordUrls.ts`, while markdown conversion and Discord entity parsing live in `frontend/src/utils/discordMessageContent.ts`, keeping URL and formatting concerns out of React components. Discord PNG/APNG stickers load from the CDN; standard Lottie stickers fall back through the authenticated backend asset route and lazy-load the light Lottie player with reduced-motion behavior. Cached Lottie JSON must initialize only after the fallback animation container is mounted, even when React Query correctly avoids another network request.
...
- The registrar container image is built from `registrar/Dockerfile`. It uses a Go 1.25 builder stage matching `registrar/go.mod`, copies a stripped static binary into an Alpine runtime, and runs as UID/GID `10001` with writable state under `/var/lib/oficina/registrar`.
...
The main bot connects to this schema after migrations have run. Runtime application users should be restricted to the DML privileges they need; MySQL admin or migration credentials should not be placed on application VMs. The backend dashboard uses existing product tables such as `birthdays`, `support_tickets`, `messages_versions`, `users`, and code-owned `store_item_settings`, plus `dashboard_sessions` for persisted dashboard login state, through a restricted `DATABASE_*` runtime user and must not create or migrate schema at startup. `users.avatar_hash` is nullable and is maintained by bot user upserts and avatar update events; do not bulk-load guild members just to backfill avatars.
...
- `store_item_settings` stores one mutable, non-negative price per `StoreItemType` key. The migration seeds every supported key, including group actions, the global color-role price, counting-punishment release, and marriage acceptance; the bot never falls back to a hard-coded price when a row is missing.
...
Purchasable bot actions are internal action pricing, not a user-facing shop. `StoreItemType` remains the fixed Java identity and behavior list, while `store_item_settings` owns its mutable price. `/group create`, channel creation, group modifications, bot additions, paid member slots, permissions, and pins resolve the database price when they create the 30-second confirmation; the quoted amount is retained through confirmation and persisted in `groups_perks.value_paid` as historical data. `/colors` uses one shared `COLOR_ROLE` price for every configured role and re-reads it before charging. Counting release and non-privileged marriage acceptance use `COUNTING_RELEASE` and `MARRIAGE`, respectively. A missing settings row fails the action closed instead of using a code default.
...
HTTP handlers receive dependencies through small interfaces instead of calling concrete service functions directly. The level card routes use an injected card renderer, and dashboard birthday, ticket, and channel-message routes use injected repositories, which keeps route behavior testable without launching Playwright or requiring MySQL in unit tests. Ticket persistence and message-history persistence are separate repository concerns even though both use the same database pool. Dashboard repositories receive one shared GORM handle layered over that pool; the app retains the underlying `database/sql` handle only for pool tuning, bounded connectivity checks, and graceful shutdown.

Backend code is split by concern: Echo handlers live in `backend/cmd/internal/http/handler/`, request/response DTOs in `backend/cmd/internal/contract/`, dashboard entities in `backend/cmd/internal/domain/entity/`, MySQL repositories in `backend/cmd/internal/domain/mysql/repository/`, and external Discord HTTP clients in `backend/cmd/internal/infrastructure/discord/`. Repository-local GORM row models map existing table shapes and convert into dependency-free domain entities; GORM tags do not leak into API/domain contracts. Keep handler methods thin; persistence and folding logic belong in repositories or focused services.
...
- `GET /channels/:channelID/messages` for authenticated folded channel history over `messages_versions`. With no anchor it returns the newest page; mutually exclusive `before`, `after`, and `around` message-ID parameters support backward, forward, and centered navigation. Responses are chronological and report `has_more_before` and `has_more_after`.
- `GET /channels/:channelID/messages/:messageID/versions` for authenticated edit history scoped to the channel.
- `GET /discord/stickers/:stickerID/lottie` for authenticated, cacheable proxying of Discord standard-sticker Lottie JSON. This exists because Discord's sticker CDN does not expose browser CORS headers for direct JSON loading.
- `GET /economy/action-costs` and `PATCH /economy/action-costs/:itemType` for authenticated `Manage Server` action-cost configuration. Updates use the existing CSRF token and last-write-wins semantics.

Shared HTTP middleware adds request IDs, recovery, JSON request logs, body limits, configured-origin CORS with credentials, and a basic in-memory rate limiter. Dashboard sessions use HttpOnly SameSite cookies scoped to the API host, persist hashed session IDs in `dashboard_sessions`, and require an `X-CSRF-Token` header for mutating dashboard API requests. Dashboard JSON contracts use `snake_case` fields; Discord snowflakes are encoded as strings to avoid JavaScript integer precision loss, and `created_at`/`updated_at` values are RFC 3339 UTC strings regardless of whether the underlying table stores Unix seconds or milliseconds. Ticket and message responses intentionally expose user IDs only (`initiator_id`, `closed_by_id`, `author_id`, `deleted_by_id`); dashboard clients resolve display names and avatar URLs through `POST /users/query`.
...
## Backend Persistence
When `DATABASE_*`, `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`, and `DISCORD_GUILD_ID` are present, the backend opens MySQL at startup, configures the standard Go connection pool, initializes GORM over that existing pool, and verifies connectivity with a bounded ping. Repository reads and writes use GORM models, clauses, and query-builder operations, including the joins and grouped cursor queries used for tickets and message history. If dashboard config is incomplete, existing health/card routes still start while dashboard auth/API calls return unavailable errors. Dashboard modules reuse product tables such as `birthdays`, `support_tickets`, `messages_versions`, `users`, and `store_item_settings`, and persist dashboard sessions in `dashboard_sessions`; the backend never calls GORM `AutoMigrate`, adds backend-owned DDL, or runs migrations at startup. Ordered schema changes remain under `database/migrations/`.

Live repository integration tests use `OFICINA_TEST_MYSQL_DSN`, create an isolated temporary schema, and exercise GORM against real MySQL behavior. This coverage is required for nullable mappings, upserts, duplicate-key translation, cursor pagination, grouped message folding, and zero-value updates that cannot be validated reliably with an in-memory substitute.
