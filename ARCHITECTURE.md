...
`/accumulator` is a `Ban Members` command for staging event prizes before payout. `/accumulator add` creates persistent `PENDING` rows in `accumulator_prizes` for money or temporary color-role prizes; `/accumulator list` renders the pending box with durable `acc:v1:` controls, six rows per page, sorted by prize type priority and then money amount descending. Rows are never deleted: refused prizes become `REJECTED`, successful payouts become `PAID`, and both individual and bulk approval store `approved_by`/`approved_at`.

Accumulator money prizes require an integer amount from 1 to 1,000,000 and are paid to the selected provider's bank balance. Color-role prizes store their duration when added, then the creator or a `Manage Server` user chooses the exact configured color role from the list controls before payout. Paying one row or approving all rows is restricted to `Manage Server`; approve-all validates every pending row before making changes, pays as a single best-effort atomic batch with rollback hooks for reversible external effects, and returns exactly one report embed.
