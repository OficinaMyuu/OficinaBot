...
The `/colors` command is the current color-role shop surface. It sends a public Components V2 container built by `ColorRoleStoreMessageFactory`: each row shows the configured role, the expiry timestamp when the user already owns it, and a buy/remove button. Those row buttons only open public confirmation messages; the final confirmation buttons still route through the existing `ColorRolePurchaseHandler` and `ColorRoleRemoveHandler`, so charging, refunding, persistence, and Discord role mutation remain centralized. Legacy `/color add`, `/color list`, `/color remove`, and `/color status` remain registered for compatibility.
