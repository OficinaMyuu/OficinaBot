...
The root domain `oficinamyuu.com.br` and `www.oficinamyuu.com.br` are expected to be attached to Cloudflare Pages outside this backend Terraform module. Public dashboard traffic uses `https://oficinamyuu.com.br/dashboard` and is served by Pages. The API hostname remains `https://api.oficinamyuu.com.br`; do not use the API host as a user-facing UI URL. The future `cdn.oficinamyuu.com.br` hostname is intentionally not configured here yet.
...
curl.exe -i https://api.oficinamyuu.com.br/auth/me
curl.exe -I https://oficinamyuu.com.br/dashboard
curl.exe -I http://<load-balancer-public-ip>/health
```

The health command should return the API response through Cloudflare, `/auth/me` should return an API auth response, and the dashboard command should return the Cloudflare Pages app. The raw load balancer command should time out or be blocked from ordinary public networks once the NSG update has applied.
