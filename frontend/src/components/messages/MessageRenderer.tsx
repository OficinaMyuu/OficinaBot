...
export const MessageRenderer = memo(function MessageRenderer({ messages }: MessageRendererProps) {
          ...
          <li className={[styles.message, isDeleted ? styles.deleted : null].filter(Boolean).join(' ')} key={message.message_id}>
            ...
            <div className={styles.body}>
              ...
              {message.message_reference_id ? (
                <div className={styles.reply}>{t('messages.replyReference', { id: message.message_reference_id })}</div>
              ) : null}

              <p className={styles.content}>
                {isDeleted ? t('messages.deleted') : content ? content : t('messages.noTextContent')}
              </p>

              {message.sticker_id ? <div className={styles.attachment}>{t('messages.sticker', { id: message.sticker_id })}</div> : null}
              {message.deleted_by ? (
                <div className={styles.audit}>{t('messages.deletedBy', { name: message.deleted_by.display_name })}</div>
              ) : null}
            </div>
          </li>
        )
